...
- **FR-006**: Each notebook MUST display 2D scatter plots showing each unique pair of input dimensions, producing $\binom{d}{2}$ subplots where $d$ is the input dimensionality. These plots show spatial coverage of sample points — no output value is plotted on the axes.
