...
4. Existing notebook has 47 cells (last cell id: `a52b2e42`)

## Implementation Order

### Phase 1: Setup (Cells 48–49)

1. **Cell 48** (Markdown): Week 7 header with strategy rationale
   - Title: `## Week 7 — SFGP Matérn-1.5 + NEI`
   - Include comparison table: Week 6 (NN+MCDropout+UCB κ=0.5) vs Week 7 (SFGP+NEI q=4)

2. **Cell 49** (Code): Imports + data loading
   ```python
...
### Phase 2: Documentation (Cell 50)

3. **Cell 50** (Markdown): Hyperparameter table with 14 entries (incl. Standardize(m=1), noise=0.1, noise floor 1e-8, distance-based selection)

### Phase 3: Model Training (Cell 51)

4. **Cell 51** (Code): GP training pipeline
   ```python
   ...
   for seed in range(N_RESTARTS):
       ...
       model.likelihood.noise = 0.1  # 10% of standardised Var(y)≈1.0
...
### Phase 4: Acquisition & Selection (Cell 52)

5. **Cell 52** (Code): NEI with q=4 + distance-based selection
   ```python
   best_model.eval()
...
### Phase 5: Visualisation (Cells 53–54)

6. **Cell 53** (Code): 3-panel surrogate plot (mean, std, 1/ℓ relevance)
   - No manual inverse transform — `model.posterior()` returns original-space values
   - 5 bars in relevance chart (not 4 as in F5)

7. **Cell 54** (Code): Convergence plot with boundary at 26.5

### Phase 6: Submission (Cell 55)

8. **Cell 55** (Code): Format and validate submission query + HP summary
...
- **Noise init = 0.1**: Because Standardize makes internal Var(y)≈1.0; `0.1 * y_raw.var()` = 0.033 would be wrong
...
| Noise init | 0.1 * Y_train.var() ≈ 0.1 | **0.1** (same value, different rationale) |
...
- 55 total cells (47 original + 8 new)
- Submission query: `0.xxxxxx-0.xxxxxx-0.xxxxxx-0.xxxxxx-0.xxxxxx`
