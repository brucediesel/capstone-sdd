...
As a data scientist, I need to fit a Gaussian Process with Matérn-5/2 kernel, log-transformed outputs, noise initialised at 0.1·Var(y), and lengthscales initialised at 0.5, so that I have a well-calibrated surrogate for acquisition.
...
1. **Given** 27 validated samples, **When** the GP is trained with Matérn-5/2 kernel (ARD), log-transformed outputs, noise=0.1·Var(y), ls=0.5, and MLL optimisation with multiple restarts, **Then** the fitted lengthscales, output scale, and noise variance are printed.
...
As a data scientist, I need to run the Noisy Expected Improvement acquisition function with q=4 candidates, ξ=0.3, optimised via 3000 Sobol starts → best 50 → L-BFGS, to propose four candidate points and select the best for submission.

**Why this priority**: Producing the submission query is the deliverable of each weekly iteration.

**Independent Test**: Run the acquisition cell and confirm 4 candidate points are returned in [0,1]⁴ with their posterior means reported.

**Acceptance Scenarios**:

1. **Given** a trained GP surrogate, **When** NEI is optimised with q=4, ξ=0.3, and 3000 Sobol initialisations narrowed to 50 L-BFGS restarts, **Then** 4 candidate points in [0,1]⁴ are returned.
2. **Given** 4 candidate points, **When** posterior means and distances to nearest observed point are computed, **Then** candidates are filtered to those with posterior mean above the median, and the farthest-from-data candidate among those is selected for submission and its coordinates are displayed.
...
- What happens if all q=4 candidates collapse to the same point? The NEI formulation with ξ=0.3 provides strong exploration pressure to diversify candidates and push away from the current best region.
...
- **FR-007**: System MUST initialise lengthscales at 0.5 (longer init encourages smoother GP prior and broader uncertainty for exploration).
- **FR-008**: System MUST initialise noise variance at 0.1 × Var(y_transformed) (moderate noise for smoother GP fit, supporting exploration).
...
- **FR-011**: System MUST use qLogNoisyExpectedImprovement (NEI) as the acquisition function with q=4 (wider candidate pool for exploration). Note: BoTorch's `eta` parameter only affects constraint smoothing, not exploration — see FR-012.
- **FR-012**: System MUST use `qLogNoisyExpectedImprovement` (log-space NEI for numerical stability). Note: BoTorch's qLogNEI has no direct ξ parameter — the `eta` parameter only affects constraint smoothing. Exploration pressure is achieved through q=4 batch diversity (FR-011), distance-based candidate selection (FR-014), longer lengthscale init at 0.5 (FR-007), and higher noise init at 0.1·Var(y) (FR-008).
...
- **FR-014**: System MUST report all q=4 candidate points with their posterior means and distances to nearest observed point. Selection strategy: filter candidates to those with posterior mean above the median of all 4 candidates, then select the candidate farthest from any existing observation (Euclidean distance in input space) for submission.
...
- **Hyperparameter Set**: lengthscales ℓ1–ℓ4 (init 0.5), signal variance σ²_f (init Var(y)), noise variance σ²_n (init 0.1·Var(y)), jitter 1e-6.
- **Acquisition Result**: q=4 candidate points from NEI, ranked by posterior mean. Wider pool maximises diversity under aggressive exploration.
...
- **SC-004**: The NEI acquisition returns exactly 4 candidate points, each with 4 coordinates in [0, 1].
...
3. **Lengthscale initialisation**: The value 0.5 applies to all 4 ARD lengthscales uniformly. This longer init encourages the GP to fit a smoother surface initially, increasing posterior uncertainty in unexplored regions and amplifying NEI's exploration pressure. The MLL optimiser adjusts from this starting point.
4. **Noise initialisation**: 0.1·Var(y) refers to the variance of the log-transformed outputs, ensuring the noise prior is calibrated to the modelling space. The higher init (vs. 0.03) prevents the GP from interpolating too tightly through observations near the current best, creating a smoother posterior that supports exploration.
5. **NEI exploration mechanism**: BoTorch's `qLogNoisyExpectedImprovement` does NOT have a ξ parameter. The `eta` parameter only controls constraint indicator smoothing (no effect when `constraints=None`). The user's ξ=0.3 exploration intent is achieved through a coherent set of exploration-promoting mechanisms: q=4 batch for candidate diversity, distance-based selection (farthest from data among above-median candidates), ℓ=0.5 lengthscale init (smoother GP = broader uncertainty), and 0.1·Var(y) noise init (prevents tight interpolation).
...
- Q: What exploration parameter ξ should NEI use to escape F5's apparent local maximum? → A: ξ=0.3 (aggressive exploration, 30× the original 0.01 setting).
- Q: What batch size q should NEI use for candidate generation? → A: q=4 (wider candidate pool under aggressive ξ=0.3; best of 4 selected for submission).
- Q: What lengthscale initialization should the GP ARD kernel use? → A: ℓ=0.5 (longer init for smoother prior and broader uncertainty, encouraging exploration).
- Q: What noise variance initialization should the GP use? → A: 0.1·Var(y) (moderate noise; smoother fit complements ℓ=0.5 and ξ=0.3 exploration setup).
- Q: How should the final submission candidate be selected from q=4 proposals? → A: Filter to candidates with posterior mean above median, then pick farthest from existing data (Euclidean distance). Balances exploration with quality.
